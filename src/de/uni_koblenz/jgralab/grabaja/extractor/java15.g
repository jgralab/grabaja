...
import de.uni_koblenz.jgralab.grabaja.extractor.adapters.CommonTokenAdapter;
import de.uni_koblenz.jgralab.grabaja.extractor.comments.*;
}
...
compilationUnit
	:
		...
		// Next we have a series of zero or more import statements ultbreit: with possible comments
		( importDefinition )*
...
annotation!
	:	a:AT i:identifier ( LPAREN! ( args:annotationArguments )? rp:RPAREN )?
		{#annotation = #(#[ANNOTATION,"ANNOTATION"], a, i, args, rp);}
	;
...
classDefinition![AST modifiers]
	:	cc:"class"
		...
		{#classDefinition = #(#[CLASS_DEF,"CLASS_DEF"],
								modifiers,cc,IDENT,tp,sc,ic,cb);}
	;
...
classField!
			...
			(
				...
				|	// A generic method/ctor has the typeParameters before the return type.
					...
					(	IDENT				// the name of the method
						...
						// parse the formal parameter declarations.
						LPAREN! param:parameterDeclarationList rp:RPAREN
	...
	|	st:"static" s3:compoundStatement
		{#classField = #(#[STATIC_INIT,"STATIC_INIT"], st, s3);}
...
explicitConstructorInvocation
	:	(typeArguments)?
		(	eci1:"this"^ LPAREN! argList RPAREN SEMI
			{#eci1.setType(CTOR_CALL);}
		|	eci2:"super"^ LPAREN! argList RPAREN SEMI
			{#eci2.setType(SUPER_CTOR_CALL);}
		)
	;
...
class JavaLexer extends Lexer("de.uni_koblenz.jgralab.grabaja.extractor.adapters.CharScannerAdapter");
options {
...

{
    ...
    /**
     * Collects all comments of a parsed class.
     */
    private ArrayList< CommentClass > commentCollection = null;

    /**
     * Sets the collection of the comments.
     */
    public void setCommentCollection( ArrayList< CommentClass > comments ){
...
SL_COMMENT
	:	"//" (~('\n'|'\r'))* ('\n'|'\r'('\n')?)
		{
		  String text = getText();
		  SingleLineCommentClass comment = new SingleLineCommentClass( getOffset(), text.length(), getLine(), getColumn(), logger );
...
DUMB_COMMENT
        :	"/**/"
        	{
		  String text = getText();
		  MultiLineCommentClass comment = new MultiLineCommentClass( getOffset(), text.length(), getLine(), getColumn(), logger );
		  commentCollection.add( comment );
        	  $setType( Token.SKIP );
        	}
        ;
...
JD_COMMENT
		...
		{
		    String text = getText();
		    JavaDocCommentClass comment = new JavaDocCommentClass( getOffset(), text.length(), getLine(), getColumn(), logger );
		    commentCollection.add( comment );
		    $setType(Token.SKIP);
		}
	;
