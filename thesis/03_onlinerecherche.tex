...
\subject{Studienarbeit Java-Extraktor für GUPRO}

\title{Onlinerecherche}

\author{Arne Baldauf \url{abaldauf@uni-koblenz.de}\\
 Nicolas Vika \url{ultbreit@uni-koblenz.de}}
...
\begin{table}[h]
  \begin{center}
    \begin{tabular}{|l|l|l|p{2,5cm}|l|}\hline 
      \textbf{Ver.} & \textbf{Status} & \textbf{Datum} & \textbf{Autor(en)} & \textbf{Erläuterung} \\ \hline
      \hline
      0.1 & WIP & 09.01.2006 & Nicolas Vika & Initiale Version\\ \hline
      0.2 & WIP & 13.01.2006 & Arne Baldauf, Nicolas Vika & Kapitel Fujaba\\ \hline
      0.5 & WIP & 16.01.2006 & Nicolas Vika & Restl. Kapitel begonnen\\ \hline
      1.0 & WIP & 17.01.2006 & Arne Baldauf, Nicolas Vika & Restl. Kapitel abgeschl.\\ \hline
      1.0 & abgen. & 17.01.2006 & Arne Baldauf & Abnahme \\ \hline
      1.1 & WIP & 03.02.2006 & Nicolas Vika & Überarbeitung \\ \hline
      1.2 & WIP & 04.02.2006 & Nicolas Vika & Überarbeitung \\ \hline
      1.3 & WIP & 04.02.2006 & Arne Baldauf & Überarbeitung \\ \hline
      2.0 & WIP & 19.02.2006 & Arne Baldauf, Nicolas Vika & Komplette Überarbeitung, Restrukturierung \\ \hline
    \end{tabular}
  \end{center}
\end{table}
...
\section{Einführung}
\subsection{Zur Struktur dieses Dokuments}
Dieses Dokument beschreibt die Ergebnisse unserer Onlinerecherche und ist in sieben Teile unterteilt.\\

Im ersten Teil beschreiben wir Struktur und Absicht dieses Dokuments, im zweiten erklären wir unser Vorgehen.
Im dritten Teil behandeln wir Parsergeneratoren und im vierten Werkzeuge, welche diese benutzen. Im fünften Abschnitt werden quelloffene Java-Compiler untersucht und der sechste Teil behandelt im speziellen Eclipse und JDT. Im siebten Abschnitt bewerten wir schließlich die beschriebenen Tools und entscheiden uns für einen vorläufigen Favoriten.\\

Jedem dieser Tools ist ein eigenes Kapitel gewidmet, in dem wir als erstes seine Historie darlegen, dann das Werkzeug kurz  charakterisieren, die existierende Dokumentation beleuchten, die Verwendung des Werkzeugs beschreiben, unsere eigenen Erfahrungen zusammenfassen und die Ergebnisse der Parsingtests darlegen. Das jeweilige Kapitel wird mit einer kurzen tabellarischen Übersicht abgeschlossen.\\

Im Anhang befinden sich die Javaquelltexte, die wir zum Testen verwendet haben.\\

\subsection{Ziel der Onlinerecherche}
Zur Extraktion von Fakten aus Javaquelltexten müssen letztere zunächst geparst werden. Parsingtechniken werden schon lange im Compilerbau verwendet und stehen in einer Vielzahl von bereits existierenden Werkzeugen zur Verfügung. Deshalb schauten wir uns zunächst in diesem Bereich um, ob nicht schon eine Lösung existierte, auf der wir den Extraktor aufbauen könnten.
Dadurch sollte unser eigener Entwicklungsaufwand möglichst weit reduziert werden.\\

Ideal wäre ein Tool, welches folgende Kriterien erfüllt:
\begin{itemize}
  \item{Java-Parsing bis einschließlich Version 5}
  \item{freie Benutzung des Werkzeugs oder der Grammatik}
  \item{Erzeugung eines AST}
  \item{Möglichkeit der Abfrage von Position und Länge eines Tokens im Code}
  \item{Verlässlichkeit der Quelle, gemessen an:}
  \begin{itemize}
    \item{Qualität und Umfang der Dokumentation}
    \item{Aktualität}
    \item{Support}
    \item{Lebendigkeit und Größe der Community}
    \item{Mögliche Unterstützung zukünftiger Java-Sprachversionen}
  \end{itemize}
  \item{Unterstützung besonderer Spracheigenschaften von Java (u. a. Reflection)}
  \item{Ausführbarkeit auf Linux, Windows und MacOS als Systemumgebung}
\end{itemize}
Zunächst betrachten wir im folgenden Abschnitt unser Vorgehen beim Test der gefundenen Software.
%Zunächst betrachten wir im folgenden Kapitel die zugrundeliegenden Parsersingverfahren.

%\section{Parsingverfahren}
%kommt noch.
%ES FEHLT  Symboltabellen\\
...
\subsection{Grober Ablauf der Tests}
...
Im Groben haben wir die Tools wie folgt behandelt:\\
...
In der ersten Phase wurden nur Quelltexte (Übersicht siehe \textit{Tabelle \ref{parsingtestphase1sources}}) geparst, die vom Java Compiler javac fehlerfrei übersetzt werden können.\\
...
In der zweiten Phase wurden nur Klassen  (Übersicht siehe \textit{Tabelle \ref{parsingtestphase2sources}}) geparst, die vom Java Compiler javac nicht fehlerfrei übersetzt werden können. Dazu haben wir Klassen genommen, die nicht aufgrund des Typecheckings, sondern aufgrund ungültiger Ausdrücken nicht kompiliert werden können. Dabei handelt es sich u. a. um ungültige Kombinationen von Modifiern und um das "`for each ( .. in .. )"'-Konstrukt, welches in einer Preview
von Java 5 noch erlaubt war, aus Kompatibilitätsgründen aber nicht mehr in der endgültigen Fassung enthalten ist.\\

In der dritten Phase schließlich sollten die Parser zeigen, dass sie mit den unterschiedlichen Zeilenumbrüchen der Betriebssystemwelt umgehen können. Dazu haben wir, für jede Art von Zeilenumbruch, den Parsern eine separate Klasse (Übersicht siehe \textit{Tabelle \ref{parsingtestphase3sources}}), sowie eine mit der Kombination aller Umbrüche, zum Parsen gegeben.\\
...
\section{Parsergeneratoren}
Bei einem Parsergenerator handelt es sich um ein Programm, welches aus einer Grammatik einen Parser erzeugt, der die Sprache, die in der Grammatik beschrieben wird, parsen kann. Da wir einen Parser für Java benötigen, werden in diesem Kapitel die in Frage kommenden Parsergeneratoren genauer untersucht. 

\subsection{JavaCC}
Eine kurze Übersicht zu den wichtigsten Daten von JavaCC findet sich in \textit{Tabelle \ref{javaccoverview}}.
\paragraph{Historie.}
Der Java Compiler Compiler wurde 1996 von Sreeni Viswanadha und Sriram Sankar, für die Sun Microsystems Laboratories, entwickelt. Seitdem ist das Werkzeug von den beiden stetig weiterentwickelt worden.\\
...
JavaCC ist ein in Java implementierter Parsergenerator, der LL(k)-Parser und die zugehörigen Lexer in Java erzeugt. Eine Vielzahl von Grammatiken für verschiedene Programmiersprachen ist auf der Hompage (\footnote{\url{http://javacc.dev.java.net/}} und \footnote{\url{http://www.cobase.cs.ucla.edu/pub/javacc/}}) verfügbar (auch für Java bis Version 5). Die Erzeugung von Bäumen geschieht nicht automatisch, sondern muss von Hand in die Grammatiken eingebaut werden. Alternativ kann dies auch durch den Einsatz von Werkzeugen, wie dem mitgelieferten JJTree oder JTB, erreicht werden, die auf der Basis von Grammatiken für JavaCC Bäume erzeugen können.
...
%Grammatik:\\
%Die Grammatik liegt in einer Datei vor, in der, zuallererst, eine Java-Klasse stehen muss, welche die mit den Eingabedaten für den %später erzeugten Parser, vorbehandeln muss. Danach werden die Tokens für die lexikalische Analyse und schließlich die Grammatik %für den Parser definiert. Hierbei\\
%ES FEHLT Mischung regulärer Ausdrücke, Javacode und BNF, semantic actions\\
%ACHTUNG Grammatik sieht komplizierter als ANTLR aus, kann aber mit unicode escape seq. umgehen - der Lexer erkennt diese\\
...
Die neuen Java 5 Ausdrücke bereiten dem Parser keine Schwierigkeiten, obschon seine Grammatik vom Februar 2004 stammt, also schon vor dem Erscheinen der endgültigen Fassung von Java 5. Allerdings stolpert der Parser in den fehlerfreien Quelltexten jedesmal über den Ausdruck aus \textit{Listing \ref{javacctest1}}.

\begin{lstlisting}[caption=Quelltextauszug der für JavaCC problematisch ist, captionpos=b, language=Java, label=javacctest1]
...
Aus diesem Grund schlägt auch das Parsen von AllowedForEachConstruct.java, TestJSR201.java und TestJSR201Berichtigt.java fehl, da auch diese Klassen o. a. Code enthalten. Wird dieser Code auskommentiert, gelingt der Parsingvorgang erfolgreich.\\
Doch auch in der Klasse Test15 stolpert der Parser über den (eigentlich korrekten) Ausdruck aus \textit{Listing \ref{javacctest2}}.
\begin{lstlisting}[caption=Quelltextauszug der für JavaCC problematisch ist, captionpos=b, language=Java, label=javacctest2]
	new <String> K<Integer>("xh");
	       ^ hier meldet der Parser einen Fehler
\end{lstlisting}
In diesem Fall kommt der Parser nicht mit Generic-Ausdrücken nach einem \texttt{new} klar. Wird der Code auskommentiert, meldet der Parser in der nächsten Zeile den selben Fehler bei einem ähnlichen Ausdruck. Nach weiterem Auskommentieren wird die Klasse endlich erfolgreich geparst.\\
...
Eine Übersicht zu den Testergebnissen findet sich in \textit{Tabelle \ref{javacctestresults}}.
...
\subsection{CUP}
Eine kurze Übersicht zu den wichtigsten Daten von CUP findet sich in \textit{Tabelle \ref{cupoverview}}.
\paragraph{Historie.}
Der Java(tm) Based Constructor of Useful Parsers (kurz CUP) wurde ursprünglich 1996 von Scott Hudson, Frank Flannery und C. Scott Ananian, an der Princeton University und Georgia Tech\footnote{\url{http://www.cs.princeton.edu/~appel/modern/java/CUP/}}, entwickelt. Die Quelltexte sind als Open Source frei verfügbar. Seit Ende 1999 wird CUP von der Technischen Universität München\footnote{\url{http://www2.cs.tum.edu/projects/cup/}} gepflegt und weiterentwickelt. Verantwortlich für die aktuellste Version (0.11a, Mai 2005) ist dort Michael Petter.
...
\paragraph{Parsingtests.}
Das Parsen der fehlerfreien Quelltexte absolviert der Parser ohne Fehl und Tadel. Dies ist allerdings nicht weiter verwunderlich, da ein Teil der Testquelltexte aus CUP entnommen wurden.\\
...
Eine Übersicht zu den Testergebnissen findet sich in \textit{Tabelle \ref{cuptestresults}}.
...
\subsection{Coco/R}
Eine kurze Übersicht zu den wichtigsten Daten von Coco/R findet sich in \textit{Tabelle \ref{cocoroverview}}.
\paragraph{Historie.}
...
\paragraph{Charakterisierung.}
Dieser LL(k)-Parsergenerator existiert in und für eine Vielzahl von Programmiersprachen. Einige Grammatiken sind auf der Homepage\footnote{\url{http://www.ssw.uni-linz.ac.at/Research/Projects/Coco/}} erhältlich - auch für Java, aber nur bis Sprachversion 1.4. Auch Coco/R bietet keine automatische Erzeugung von Bäumen. Diese Funktionalität muss, als Aktionscode zu den entsprechenden Produktionen, von Hand implementiert werden.
...
\paragraph{Verwendung.}
Auch die Verwendung von Coco/R erfolgt über die Kommandozeile. Beim Aufruf muss als Parameter die Grammatik angegeben werden. Die so erzeugten Scanner und Parser müssen dann in einem eigenen Programm aufgerufen werden (benötigen aber keine Komponenten von Coco/R zur Laufzeit).\\
...
Eine Übersicht zu den Testergebnissen findet sich in \textit{Tabelle \ref{cocortestresults}}.
\begin{table}[h]
    \begin{tabular}{|p{6cm}|l|p{7,5cm}|} \hline 
      \textbf{Testquelltext} & \textbf{OK} & \textbf{Grund}\\ \hline
      \hline
...
\subsection{Cocktail}
Eine kurze Übersicht zu den wichtigsten Daten von Cocktail findet sich in \textit{Tabelle \ref{cocktailoverview}}.
\paragraph{Historie.}
Die Entwicklung der ersten Version der Cocktail Toolbox begann 1988 an der GMD Forschungsstelle der Universität Karlsruhe. Nachdem die Forschungsstelle Ende 1993 geschlossen wurde, übernahm Josef Grosch die Rechte und gründete die Firma CoCoLab \footnote{\url{http://www.cocolab.org/}}. Die Software ist inzwischen kostenpflichtig und erhält in größeren aber regelmäßigen Abständen Updates.

\paragraph{Charakterisierung.}
Die Cocktail Toolbox beinhaltet eine Reihe von Werkzeugen für den Compilerbau, die Programmtransformation und die Programmanalyse. Ziel bei der Entwicklung dieser Werkzeuge war eine gesteigerter Umfang und eine gesteigerte Leistungsfähigkeit im Vergleich zu Lex / Yacc.\\
...
\paragraph{Verwendung.}
Da die Nutzung kostenpflichtig ist, steht auf der Homepage nur eine beschränkt lauffähige Version zur Verfügung. Diese kommt ohne Oberfläche und besteht im Wesentlichen aus einer Sammlung der Kommandozeilenwerkzeuge. Da aber auch keine Grammatiken dabei sind, entfällt der Test der Werkzeuge.\\
...
\paragraph{Parsingtests.}
Das Parsen der fehlerfreien Quelltexte geschieht fast problemlos. Einzig die Klasse Test15 bereitet dem Parser Schwierigkeiten - teilweise führte dies sogar zum Absturz des Programms.\\

Beim Parsen der fehlerhaften Quelltexte findet der Parser nur den nicht mehr erlaubten, "`for each ( .. in .. )"'-Ausdruck. Die abstrakte Enum-Konstruktion und ungültige Modifierkombinationen bemerkt der Parser nicht.\\

Die Parsingtests der Quelltexte mit problematischen Newlines verliefen hingegen positiv.\\

Eine Übersicht zu den Testergebnissen findet sich in \textit{Tabelle \ref{cocktailtestresults}}.
\begin{table}[h]
    \begin{tabular}{|p{6cm}|l|p{7,5cm}|} \hline 
      \textbf{Testquelltext} & \textbf{OK} & \textbf{Grund}\\ \hline
      \hline
...
\subsection{ANTLR}
Eine kurze Übersicht zu den wichtigsten Daten von ANTLR findet sich in \textit{Tabelle \ref{antlroverview}}.
\paragraph{Historie.}
...
% diese Grammatik nicht als fehlerfrei erachtet werden kann.\\

\paragraph{Position \& Länge der Tokens im Quelltext.}
Laut API-Beschreibung lässt sich die Position des Tokens im Quelltext nicht direkt abrufen. Lediglich Zeile, Spalte und Länge (über die Länge des Strings) des Tokens sind einsehbar.\\
...
Alle Testquelltexte mit fehlerfreien Konstrukten wurden von diesem Parser akzeptiert, ausgenommen diejenigen, welche "`enum"'-Konstrukte enthalten. Offensichtlich ist die vorhandene Grammatik nicht vollständig und erkennt diesen Ausdruck nicht.\\
...
Eine Übersicht zu den Testergebnissen findet sich in \textit{Tabelle \ref{antlrtestresultsstahl}}.

\paragraph{Parsingtests - Parser nach Grammatik von Michael Studman.}
Alle Testquelltexte mit fehlerfreien Konstrukten wurden von diesem Parser akzeptiert, im Gegensatz zur Grammatik von Stahl auch solche mit "`enum"'-Ausdrücken.\\
...
Eine Übersicht zu den Testergebnissen findet sich in \textit{Tabelle \ref{antlrtestresultsstudman}}.
\begin{table}[h]
    \begin{tabular}{|p{6cm}|l|p{7,5cm}|} \hline 
      \textbf{Testquelltext} & \textbf{OK} & \textbf{Grund}\\ \hline
      \hline
...
\section{Werkzeuge, die Parsergeneratoren benutzen}
In diesem Kapitel werden Tools untersucht, welche auf einen Parsergenerator aufbauen oder mit einem durch einen Generator erzeugten Parser arbeiten und dafür eigene Java-Grammatiken mitbringen.

\subsection{JAbstract - A full Abstract Syntax and Parser for Java}
Eine kurze Übersicht zu den wichtigsten Daten von JAbstract findet sich in \textit{Tabelle \ref{jabstractoverview}}.
\paragraph{Historie.}
...
\paragraph{Literatur.}
Die Dokumentation zu diesem Projekt ist leider sehr spärlich. Auf der Homepage \footnote{\url{http://www.doc.gold.ac.uk/~mas01sd/jabstract/}} kann man nur die Quelltexte und die zugehörige, mit Javadoc erzeugte API-Beschreibung einsehen.
...
\subsection{JRefactory}
Eine kurze Übersicht zu den wichtigsten Daten von JRefactory findet sich in \textit{Tabelle \ref{jrefactoryoverview}}.
\paragraph{Historie.}
Die Entwicklung von JRefactory wurde 1999 von Chris Seguin begonnen und 2002 von Mike Atkinson übernommen. Die letzte veröffentlichte Version stammt von Ende 2003 und auch die Homepage ist seit diesem Zeitpunkt nicht mehr wesentlich aktualisiert worden.\\
...
\paragraph{Charakterisierung.}
Bei JRefactory handelt es sich um ein Tool, welches Refactorings von Javaquelltexten unterstützt. Es kann als reines Kommandozeilenwerkzeug benutzt werden, bringt aber auch eine eigene Oberfläche mit. Ferner existieren PlugIns für diverse IDEs die Java unterstützen.\\
Interressant für uns sind allerdigs nicht die Refactoring-Funktionen, sondern der intergrierte Javaparser. Dieser wird als separates Paket zum Download angeboten. Dabei handelt es sich um einen Parser, der auf einer eigenen JavaCC-Grammatik basiert. Diese soll auch  Sprachkonstrukte aus Java Version 5 kennen und einen AST erzeugen. So behandeln wir im Rest des Kapitels nur den Javaparser genauer.

\paragraph{Literatur.}
Zu JRefactory existiert keine wirklich brauchbare Dokumentation. Die Homepage \footnote{\url{http://jrefactory.sourceforge.net/}} beschränkt sich eigentlich nur auf eine Beschreibung der Features.
...
\paragraph{Eigene Erfahrungen.}
Da der Parser in einem vorkompilierten JAR-Paket geliefert wird, entfällt die Erzeugung mit JavaCC. Es wird zwar explizit darauf hingewiesen, dass Informationen zur Benutzung auf der Homepage verfügbar seien - leider ist dies nicht der Fall.\\
...
\subsection{FUJABA}
Eine kurze Übersicht zu den wichtigsten Daten von FUJABA findet sich in \textit{Tabelle \ref{fujabaoverview}}.
\paragraph{Historie.}
FUJABA steht für "`From UML to Java and back again"' und wurde 1997 von Tomas Klein, Ulrich Nickel, Jörg Niere und Albert Zündorf an der Universität Paderborn entwickelt. Seitdem unterliegt es einer ständigen Weiterentwicklung. Die aktuelle Version stammt vom März 2005, allerdings ist für März 2006 schon die nächste Version angekündigt.\\
...
Prototypen erstellen. Der umgekehrte Weg ist ebenfalls möglich, sprich Java Quelltexte zu parsen und (bis zu einem gewissen Grad) in UML Diagramme umsetzen zu können. Dabei wird auch die Verwendung von Patterns durch eine eigene Spezifikationssprache unterstützt.\\
...
\paragraph{Literatur.}
Auf der Homepage \footnote{\url{http://www.fujaba.de/}}von FUJABA existiert eine ausführliche Dokumentation zu dessen Verwendung, für Entwickler existieren hingegen nur eine API-Beschreibung von FUJABA und ein paar Einträge im Wiki. Bei Fragen kann man sich an eine Mailingliste wenden.
...
\paragraph{Eigene Erfahrungen.}
Durch Ausprobieren und Entpacken der PlugIn-Packages von Fujaba haben wir herausgefunden, dass sich FUJABA einer freizugänglichen JavaCC-Grammatik bedient. Aus dieser wurde erst mit jtb (Java Tree Builder, Quelle: \url{http://compilers.cs.ucla.edu/jtb/}) eine weitere Grammatik und Klassen für einen AST erzeugt. Aus der neuen Grammatik wurde schließlich mit JavaCC der Parser generiert, der den AST aufbaut.\\
...
\paragraph{Position \& Länge der Token im Quelltext.}
Da es sich um eine JavaCC-Grammatik handelt, gilt auch hier alles was bereits im Kapitel JavaCC geschrieben wurde.
...
\section{Quelloffene Java-Compiler Implementationen}
...
\subsection{GCJ}
Eine kurze Übersicht zu den wichtigsten Daten von GCJ findet sich in \textit{Tabelle \ref{gcjoverview}}.
\paragraph{Historie.}
...
\subsection{Java Espresso}
Eine kurze Übersicht zu den wichtigsten Daten von ANTLR findet sich in \textit{Tabelle \ref{javaespressooverview}}.\\
Die Arbeitsweise dieses Compilers ist ausführlich auf der Homepage \footnote{\url{http:/types.bu.edu/Espresso/JavaEspresso.html/}} dokumentiert. Dieser baut beim Parsen einen AST auf. Danach wird das Type-Checking durchgeführt, indem von jedem Knoten die Methode typeCheck() aufgerufen wird (die Knoten sind in Klassen beschrieben). Um dabei beispielsweise die Sichtbarkeiten aufzulösen, bedient sich Espresso einer eigenen Typhierarchie.
\begin{table}[h]
  %\begin{center}
    \begin{tabular}{|l|p{11,5cm}|}\hline 
      \textbf{Kriterium} & \textbf{Rechercheergebnis} \\ \hline
...
%\paragraph{Historie.}
%Hier gibt es nichts zu sagen, was nicht bereits unter der allgemeinen Historie von Eclipse genannt wurde.\\
...
\subsection{JDT Core}
Eine kurze Übersicht zu den wichtigsten Daten von JDT Code findet sich in \textit{Tabelle \ref{jdtcoreoverview}}.
\paragraph{Historie.}
...
\textit{JAbstract} bietet zwar die Möglichkeit zur Erzeugung eines AST, entfällt aber aufgrund der völlig veralteten Grammatik und der mangelnden Weiterentwicklung.\\
\textit{JRefactory} unterstützt zwar Parsing von Java 1.5, ist aber, aufgrund der mangelhaften Dokumentation des Parsers (welche schon die Parsing-Tests vereitelte) und der Inaktivität der Entwickler seit fast zwei Jahren, als stabile Grundlage ebenfalls ausgeschlossen.\\
\textit{Fujaba} wird aktiv weiterentwickelt, liefert aber einen AST, der bei weitem nicht vollständig ist. Desweiteren wird derzeit lediglich Java bis Version 1.4 unterstützt, so dass dieses Tool technisch ungeeignet ist.\\
\textit{GCJ} entfällt aufgrund der Tatsache, dass sowohl der Parsergenerator als auch der erzeugte Parser in C vorliegen.\\

\textit{Eclipse} bzw. das \textit{JDT-Core-Paket} besitzen zwar weitgehend alle benötigten Funktionen, implizieren aber eine Implementierung des Faktenextraktors als Eclipse-Plugin, so dass eine Integration als Kommandozeilenwerkzeug in Gupro entfällt.\\
\textit{Cocktail} entfällt aufgrund der Tatsache, dass es kostenpflichtig ist und mit den verfügbaren Demoversionen keine genaue Evaluierung der Funktionalität möglich ist.\\
\textit{Coco/R} unterstützt derzeit nur Java bis Version 1.4, ist nur begrenzt dokumentiert und bietet keine automatische Erzeugung eines AST an.\\
Somit verbleiben noch JavaCC, CUP und ANTLR in der engeren Auswahl. \textit{JavaCC} ist sehr gut dokumentiert und besitzt eine gute Community, bietet aber ebenfalls keine automatische Erzeugung des AST an. Darüber hinaus ist die derzeit verfügbare Grammatik für Java 1.5 fehlerhaft.\\
\textit{CUP} erzeugt ebenfalls keinen AST, ist nur begrenzt dokumentiert und liefert darüber hinaus keinen eigenen Lexer mit.\\
\textit{ANTLR} erzeugt, im Gegensatz zu den letzten drei genannten Projekten, auch automatisch einen AST und entspricht lediglich bei der Abfrage der Position des Token nicht ganz unseren Vorstellungen.\\
...
Bei \textit{JavaCC} stünde zusätzlich eine Funktionalität zur Erzeugung des AST sowie eine Korrektur der Grammatik aus.\\
Bei \textit{CUP} wäre ebenfalls eine Implementierung der AST-Erzeugung und die Erstellung eines bracuhbaren Lexers notwendig.\\
Für \textit{Coco/R} müsste eine Grammatik für Java 1.5 erzeugt werden und ebenfalls die Funktionalität der AST-Erzeugung implementiert werden.\\
Bei \textit{ANTLR} wäre es lediglich notwendig, die Funktion zur Positionsabfrage zu modifizieren.\\
...
\begin{appendix}
\section{Javaquelltexte für Tests}
\begin{lstlisting}[caption=Eric.java, captionpos=b, language=Java, label=eric.java]
/**
...
\end{lstlisting}
\hline \vspace{3mm}
\begin{lstlisting}[caption=EscapeSequences.java, captionpos=b, language=Java, label=escapesequences.java]
/**
...
\end{lstlisting}
\hline \vspace{3mm}
\begin{lstlisting}[caption=AllowedGenericConstruct.java, captionpos=b, language=Java, label=allowedgenericconstruct.java]
/**
...
\begin{lstlisting}[caption=AllowedForEachConstructs.java, captionpos=b, language=Java, label=allowedforeachconstructs.java]
import java.io.*;
/**
...
\begin{lstlisting}[caption=AnotherAllowedForEachConstruct.java, captionpos=b, language=Java, label=anotherallowedforeachconstruct.java]
import java.io.*;
/**
 * Diese Klasse benutzt ein ForEach Konstrukt, welches in der
...
\begin{lstlisting}[caption=Test15.java, captionpos=b, language=Java, label=test15.java]
...
\begin{lstlisting}[caption=TestJSR201Berichtigt.java, captionpos=b, language=Java, label=testjsr201berichtigt.java]
/**
...
\begin{lstlisting}[caption=AbstractEnumShouldGiveParseError.java, captionpos=b, language=Java, label=abstractenumshouldgiveparseerror.java]
/**
...
\begin{lstlisting}[caption=AbstractFinalShouldGiveParseError.java, captionpos=b, language=Java, label=abstractfinalshouldgiveparseerror.java]
/**
...
\begin{lstlisting}[caption=ForEachInShouldGiveParseError.java, captionpos=b, language=Java, label=foreachinshouldgiveparseerror.java]
/**
...
\begin{lstlisting}[caption=PublicPrivateShouldGiveParseError.java, captionpos=b, language=Java, label=publicprivateshouldgiveparseerror.java]
/**
...
\begin{thebibliography}{breitestes Label}
  \bibitem[Hinterwäller05]{DipBoHi} \emph{Diplomarbeit Metamodell basierte Spezifikation von Refactorings}, Bodo Hinterwäller,  
  Universität Koblenz, 2005

  \bibitem[JaLaSpec1]{HoJaLaSpec1} \emph{The Java Language Specification, First Ed.}, James Gosling, Bill Joy, Guy Steele, Sun Microsystems, 1996\\  
  \url{http://java.sun.com/docs/books/jls/first_edition/html/index.html}\\

  \bibitem[JaLaSpec2]{HoJaLaSpec2} \emph{The Java Language Specification, Second Ed.}, James Gosling, Bill Joy, Guy Steele, Gilad Bracha, Sun Microssystems, 2000\\
  \url{http://java.sun.com/docs/books/jls/second_edition/html/j.title.doc.html}\\

  \bibitem[JaLaSpec3]{HoJaLaSpec3} \emph{The Java Language Specification, Third Ed.}, Gilad Bracha, Sun Microsystems, 2005\\  
  \url{http://java.sun.com/docs/books/jls/third_edition/html/j3TOC.html}\\
  
  %\bibitem[jay]{Hojay} \url{www.informatik.uni-osnabrueck.de/alumni/bernd/jay/staff/design/de/Artikel.htmld}
  %\bibitem[Stratego]{HoStratego} \url{www.program-transformation.org/Stratego/WebHome}
  %\bibitem[Accent]{HoAccent} \url{http://accent.compilertools.net/}
\end{thebibliography}
