...
public class TypeDefinitionFactory extends SubgraphFactory {
	...
	/**
	 * Creates an edge between a super type specification and it's inherited
	 * type (class or interface).
	 * 
	 * @param parentVertex
	 *            The type.
	 * @param qualifiedNameVertex
	 *            The qualified type (specification).
	 * @param beginAST
	 ...
	 *            type.
	 */
	public void attachSuperClass(Vertex parentVertex,
			QualifiedName qualifiedNameVertex, AST beginAST, AST endAST) {
		if (parentVertex instanceof ClassDefinition) {
			IsSuperClassOfClass isSuperClassOfClassEdge = programGraph
					.createIsSuperClassOfClass(qualifiedNameVertex,
							(ClassDefinition) parentVertex);
			Utilities.fillEdgeAttributesFromASTDifference(
					isSuperClassOfClassEdge, beginAST, endAST);
		} else if (parentVertex instanceof InterfaceDefinition) {
			IsSuperClassOfInterface isSuperClassOfInterfaceEdge = programGraph
					.createIsSuperClassOfInterface(qualifiedNameVertex,
							(InterfaceDefinition) parentVertex);
			Utilities.fillEdgeAttributesFromASTDifference(
					isSuperClassOfInterfaceEdge, beginAST, endAST);
		}

	}

	/**
	 * Creates an edge between an interface type specification and it's
	 * implementing type (class or enum).
	 * 
	 * @param parentVertex
	 *            The type.
	 * @param qualifiedNameVertex
	 ...
	 *            type.
	 */
	public void attachInterface(Vertex parentVertex,
			QualifiedName qualifiedNameVertex, AST beginAST, AST endAST) {
		if (parentVertex instanceof ClassDefinition) {
			IsInterfaceOfClass isInterfaceOfClassEdge = programGraph
					.createIsInterfaceOfClass(qualifiedNameVertex,
							(ClassDefinition) parentVertex);
			Utilities.fillEdgeAttributesFromASTDifference(
					isInterfaceOfClassEdge, beginAST, endAST);
		} else if (parentVertex instanceof EnumDefinition) {
			IsInterfaceOfEnum isInterfaceOfEnumEdge = programGraph
					.createIsInterfaceOfEnum(qualifiedNameVertex,
							(EnumDefinition) parentVertex);
			Utilities.fillEdgeAttributesFromASTDifference(
					isInterfaceOfEnumEdge, beginAST, endAST);
		}
	}
	 ...
	 */
	public void attachClassBlock(Block blockVertex,
			ClassDefinition classDefinitionVertex, AST beginAST, AST endAST) {
		IsClassBlockOf isClassBlockOfEdge = programGraph.createIsClassBlockOf(
				blockVertex, classDefinitionVertex);
		Utilities.fillEdgeAttributesFromASTDifference(isClassBlockOfEdge,
				beginAST, endAST);
	}
	 ...
	 *            The AST element representing the last element of the body.
	 */
	public void attachInterfaceBlock(Block blockVertex,
			InterfaceDefinition interfaceDefinitionVertex, AST beginAST,
			AST endAST) {
		IsInterfaceBlockOf isInterfaceBlockOfEdge = programGraph
				.createIsInterfaceBlockOf(blockVertex,
						interfaceDefinitionVertex);
		Utilities.fillEdgeAttributesFromASTDifference(isInterfaceBlockOfEdge,
				beginAST, endAST);
	}
	 ...
	 *            The AST element representing the last element of the body.
	 */
	public void attachEnumBlock(Block blockVertex,
			EnumDefinition enumDefinitionVertex, AST beginAST, AST endAST) {
		IsEnumBlockOf isEnumBlockOfEdge = programGraph.createIsEnumBlockOf(
				blockVertex, enumDefinitionVertex);
		Utilities.fillEdgeAttributesFromASTDifference(isEnumBlockOfEdge,
				beginAST, endAST);
	}
	 ...
	 *            The AST element representing the last element of the body.
	 */
	public void attachAnnotationBlock(Block blockVertex,
			AnnotationDefinition annotationDefinitionVertex, AST beginAST,
			AST endAST) {
		IsAnnotationBlockOf isAnnotationBlockOfEdge = programGraph
				.createIsAnnotationBlockOf(blockVertex,
						annotationDefinitionVertex);
		Utilities.fillEdgeAttributesFromASTDifference(isAnnotationBlockOfEdge,
				beginAST, endAST);
	}
	 ...
	 *            definition.
	 */
	public void attachType(Type typeVertex, SourceUsage sourceUsageVertex,
			AST beginAST, AST endAST) {
		IsExternalDeclarationIn isExternalDeclarationInEdge = programGraph
				.createIsExternalDeclarationIn(typeVertex, sourceUsageVertex);
		Utilities.fillEdgeAttributesFromASTDifference(
				isExternalDeclarationInEdge, beginAST, endAST);
	}

	/**
	 * Creates an edge between a type definition and the statement (should be a
	 * Block, Case or Default) it is defined in (for nested classes).
	 * 
	 * @param typeVertex
	 *            The type definition.
	 ...
	 *            definition.
	 */
	public void attachType(Type typeVertex, Statement statementVertex,
			AST beginAST, AST endAST) {
		IsTypeIn isTypeInEdge = programGraph.createIsTypeIn(typeVertex,
				statementVertex);
		Utilities.fillEdgeAttributesFromASTDifference(isTypeInEdge, beginAST,
				endAST);
	}
