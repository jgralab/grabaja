...
/**
 * Holds references to all vertices needed to resolve types in TGraph and to make sure
 * that certain vertices are not created more than once (this functionality is implemented
 * by factories in package <code>javaextractor.factories</code>).<br>
...
public class SymbolTable{
     ...
     * @param programGraph TGraph the symbol table should belong to.
     */
...
//--- type definitions ---------------------------------------------------------------------------------------
     ...
     * @param fullyQualifiedName Fully qualified name of type definition to add.
     * @param typeVertex Vertex representing type definition.
     */
    ...
    /**
     * Checks if a vertex representing type definition with given name is stored in symbol table.
     * @param fullyQualifiedName Fully qualified name of type definition to check for.
    ...
    /**
     * @param fullyQualifiedName Fully qualified name of type definition to get.
...
//--- packages (unique in graph) ---------------------------------------------------------------------------------------
    ...
    /**
     * Checks if a vertex representing package definition with given name is stored in symbol table.
     * @param fullyQualifiedName Fully qualified name of package definition to check for.
     * @return true if vertex is stored in symbol table, false otherwise.
     */
	 ...
	 */
    public void addJavaPackage( String fullyQualifiedName, JavaPackage javaPackageVertex ){
        javaPackages.put( fullyQualifiedName, javaPackageVertex );
    }

    /**
     * @param fullyQualifiedName Fully qualified name of package definition to get.
     * @return Vertex representing package definition given by fully qualified name.
     */
...
//--- integer constants (unique in graph) ---------------------------------------------------------------------------------------
    ...
    public void addIntegerConstant( String literal, IntegerConstant integerConstantVertex ){
...
//--- string constants (unique in graph) ---------------------------------------------------------------------------------------
    ...
    public void addStringConstant( String literal, StringConstant stringConstantVertex ){
...
//--- float constants (unique in graph) ---------------------------------------------------------------------------------------
    ...
    public void addFloatConstant( String literal, FloatConstant floatConstantVertex ){
...
//--- double constants (unique in graph) ---------------------------------------------------------------------------------------
    ...
    public void addDoubleConstant( String literal, DoubleConstant doubleConstantVertex ){
...
//--- boolean constants (unique in graph) ---------------------------------------------------------------------------------------
    ...
    public boolean hasBooleanTrueConstant(){
        if( booleanTrueConstant == null ) return false;
        return true;
    }
    ...
    public boolean hasBooleanFalseConstant(){
        if( booleanFalseConstant == null ) return false;
        return true;
    }
...
//------ null expression (unique in graph) -------------------
    ...
    public boolean hasNull(){
        if( nullVertex == null ) return false;
        return true;
    }
...
//---- empty statement (unique in graph ) ------------------
    ...
    public boolean hasEmptyStatement(){
        if( emptyStatementVertex == null ) return false;
        return true;
    }
...
//--- scopes (for type resolving) ---------------------------------------------------------------------------------------
	...
	/**
	 * A structure holding all scope-to-parent-scope relationships.
	 * Key in the hashmap is the scope vertex, whereas the data for the key is the respective parent scope.
	 * This information only exists during treewalk of a file and is resetted with every new file that is parsed.
	 */
	private HashMap< Vertex, Vertex > parentScopes = new HashMap< Vertex, Vertex >();

	/**
	 * Adds scope hierarchy info.
	 * @param currentScope A vertex which represents an element that has its own scope.
	 * @param parentScope The vertex representing the element with the parent scope of the current.
	 */
	public void addScopeInfo( Vertex currentScope, Vertex parentScopeOfCurrent ){
		if ( currentScope != null )	parentScopes.put( currentScope, parentScopeOfCurrent );
	}

	/**
	 * Gets the parent scope of a scope.
	 * @param currentScope A vertex which represents an element that has its own scope.
	 * @return The vertex representing the element with the parent scope; null if there is none or the element cannot be found.
	 */
	public Vertex getParentScope( Vertex currentScope ){
		if ( ( currentScope != null ) && parentScopes.containsKey( currentScope ) ) return parentScopes.get( currentScope );
    	return null;
	}
...
//--- fields (locally visible) ---------------------------------------------------------------------------------------
	...
	/**
	 * A structure holding references to all found field declarations in their according scopes.
	 * Key vertex in the outer hashmap is the scope of the field declaration. Data for every key is a hashmap,
	 * where the key string is the name of the field. Data for every key in the inner hasmap is the reference
	 * to the according field declaration.
	 * This information only exists during treewalk of a file and is resetted with every new file that is parsed.
	 */
	...
	/**
	 * Adds a local field declaration to the current class (or interface, enum, etc.) and scope.
	 * @param declaredField The vertex representing the field.
	 * @param scopeOfField The scope the field is declared in.
	 */
	public void addLocalFieldDeclaration( FieldDeclaration declaredField, Vertex scopeOfField ){
		String nameOfField;
		if ( declaredField instanceof VariableDeclaration )
			nameOfField = ( ( Identifier )( ( VariableDeclaration )declaredField ).getFirstIsVariableNameOf( EdgeDirection.IN ).getAlpha() ).getName();
		else
			nameOfField = ( ( Identifier ) ( ( ParameterDeclaration )declaredField ).getFirstIsParameterNameOf( EdgeDirection.IN ).getAlpha() ).getName();
		if ( declaredLocalFieldsInScopes.containsKey( scopeOfField ) )
			declaredLocalFieldsInScopes.get( scopeOfField ).put( nameOfField, declaredField );
		else{
	...
	/**
	 * Gets a local field declaration from the current class (or interface, enum, etc.) and scope.
	 * If there is none in the current scope, parent scopes are being searched recursively.
	 * @param fieldName The name of the field.
	 * @param scopeToSearch The initial scope to search in.
	 * @return The vertex representing the field declaration; null if there is none or the element cannot be found.
	 */
	public FieldDeclaration getFieldDeclarationFromScope( String fieldName, Vertex scopeToSearch ){
		if ( ( scopeToSearch == null ) || ( fieldName == null ) ) return null;
		HashMap< String, FieldDeclaration > declaredFieldsInSameScope = declaredLocalFieldsInScopes.get( scopeToSearch );
		if( declaredFieldsInSameScope == null )	return getFieldDeclarationFromScope( fieldName, getParentScope( scopeToSearch ) );
		if( declaredFieldsInSameScope.containsKey( fieldName ) ) return declaredFieldsInSameScope.get( fieldName );
        return getFieldDeclarationFromScope( fieldName, getParentScope( scopeToSearch ) );
	}

//--- fields (globally visible) ---------------------------------------------------------------------------------------

	/**
	 * Holds all global variable declarations from the current class (or interface, enum, etc.).
	 * This information only exists during treewalk of a file and is resetted with every new file that is parsed.
	 */
	private HashMap< String, VariableDeclaration > declaredGlobalVariables = new HashMap< String, VariableDeclaration >();

	/**
	 * Adds a global variable declaration from the current class (or interface, enum, etc.).
	 * @param nameOfVariable The name of the variable.
	 * @param variableDeclarationVertex The vertex representing the variable declaration.
	 */
	public void addGlobalVariableDeclaration ( String nameOfVariable, VariableDeclaration variableDeclarationVertex ){
		if ( ( nameOfVariable != null ) && ( variableDeclarationVertex != null ) )
			declaredGlobalVariables.put( nameOfVariable, variableDeclarationVertex );
	}

	/**
	 * Gets a global variable declaration from the current class (or interface, enum, etc.).
	 * @param nameOfVariable The name of the variable.
	 * @return The vertex representing the variable declaration; null if there is none or the element cannot be found.
	 */
	public VariableDeclaration getGlobalVariableDeclaration( String nameOfVariable ){
		if ( ( nameOfVariable != null) && declaredGlobalVariables.containsKey( nameOfVariable ) ) return declaredGlobalVariables.get( nameOfVariable );
		return null;
	}
...
//--- field access (to resolve) ---------------------------------------------------------------------------------------
	...
	/**
	 * An arraylist of field access vertices containing those vertices that have not yet been
	 * linked to the according definition.
	 */
	 ...
	 * @param accessOfVariable The field access element to be added.
	 */
	public void addAccessOfUndeclaredInternalVariable( FieldAccess accessOfVariable ){
		if ( accessOfVariable != null )	accessOfUndeclaredInternalVariables.add( accessOfVariable );
	}
	...
	public FieldAccess getAccessOfUndeclaredInternalVariable( int index ){
		if( ( index >= 0 ) && ( index < accessOfUndeclaredInternalVariables.size() ) )
	...
	/**
	 * A structure holding references to all found variable declarations.
	 * Key string in the outer hashmap is the fully qualified name of the class. Data for every key is a hashmap,
	 * where the key string is the name of the variable. Data for every key in the inner hasmap is the reference
	 * to the according variable declaration.
	 */
	 ...
	 * @param fullyQualifiedNameOfContainingType The fully qualified Name of the class which contains the variable.
	 * @param variableDeclaration The vertex of the method declaration.
	 */
	public void addVariableDeclaration( String fullyQualifiedNameOfContainingType, VariableDeclaration variableDeclaration ){
		if ( ( fullyQualifiedNameOfContainingType != null) && ( variableDeclaration != null ) ){
			String variableName = ( ( Identifier ) variableDeclaration.getFirstIsVariableNameOf( EdgeDirection.IN ).getAlpha() ).getName();
			if ( declaredVariablesInClasses.containsKey( fullyQualifiedNameOfContainingType ) )
				declaredVariablesInClasses.get( fullyQualifiedNameOfContainingType ).put( variableName, variableDeclaration );
			else{
				HashMap< String, VariableDeclaration > declaredVariablesInCurrentClass = new HashMap< String, VariableDeclaration >();
				declaredVariablesInCurrentClass.put( variableName, variableDeclaration );
				declaredVariablesInClasses.put( fullyQualifiedNameOfContainingType, declaredVariablesInCurrentClass );
			}
		}
	}
	 ...
	 * @param fullyQualifiedNameOfContainingType The fully qualified Name of the class which contains the variable.
	 * @param variableName The name of the variable in the class.
	 * @return The variable; null if there is none or the class cannot be found.
	 */
	public VariableDeclaration getVariableDeclaration( String fullyQualifiedNameOfContainingType, String variableName ){
		if ( ( fullyQualifiedNameOfContainingType != null ) && ( variableName != null ) && declaredVariablesInClasses.containsKey( fullyQualifiedNameOfContainingType ) && declaredVariablesInClasses.get( fullyQualifiedNameOfContainingType ).containsKey( variableName ) )
			return declaredVariablesInClasses.get( fullyQualifiedNameOfContainingType ).get( variableName );
		return null;
	}

	/**
	 * A structure holding references to all found enum constant declarations in the current file.
	 * Key string in the hashmap is the name of the constant. Data for every key is the reference
	 * to the according enum constant.
	 */
	 ...
	 * Key string in the outer hashmap is the fully qualified name of the type (should be an enum) containing the
	 * constant. Data for every key is a hashmap, where the key string is the name of the constant. Data for
	 * every key in the inner hasmap is the reference to the according enum constant.
	 */
	 ...
	 * @param fullyQualifiedNameOfContainingType The fully qualified Name of the enum which contains the constant.
	 * @param enumConstantToAdd The vertex of the enum constant.
	 */
	public void addEnumConstant( String fullyQualifiedNameOfContainingType, EnumConstant enumConstantToAdd ){
		if ( ( fullyQualifiedNameOfContainingType != null ) && ( enumConstantToAdd != null ) ){
			String constantName = ( ( Identifier )( enumConstantToAdd.getFirstIsEnumConstantNameOf( EdgeDirection.IN ).getAlpha() ) ).getName();
			currentEnumConstants.put( constantName, enumConstantToAdd );
			if ( enumConstants.containsKey( fullyQualifiedNameOfContainingType ) ){
				enumConstants.get( fullyQualifiedNameOfContainingType ).put( constantName, enumConstantToAdd );
			}
			else{
				...
				enumConstants.put( fullyQualifiedNameOfContainingType, currentInnerMap );
			}
		}
	}
	 ...
	 * @param fullyQualifiedNameOfContainingType The fully qualified Name of the enum which contains the constant.
	 * @param constantName The name of the constant in the enum.
	 * @return The constant; null if there is none or the enum cannot be found.
	 */
	public EnumConstant getEnumConstant( String fullyQualifiedNameOfContainingType, String constantName ){
		if ( ( fullyQualifiedNameOfContainingType != null ) && ( constantName != null ) && enumConstants.containsKey( fullyQualifiedNameOfContainingType ) && enumConstants.get( fullyQualifiedNameOfContainingType ).containsKey( constantName ) )
			return enumConstants.get( fullyQualifiedNameOfContainingType ).get( constantName );
		return null;
	}
	...
	public EnumConstant getEnumConstantFromCurrent( String constantName ){
		if ( ( constantName != null ) && currentEnumConstants.containsKey( constantName ) ) return currentEnumConstants.get( constantName );
		return null;
	}

	/**
	 * Holds references to all field access vertices. Data for every key denotes if the field has already been processed by the global resolver.
	 */
	private HashMap< FieldAccess, Boolean > fieldAccessMap = new HashMap< FieldAccess, Boolean >();

	/**
	 * @return The total amount of field access vertices stored in the symbol table.
	 */
	 ...
	 * @param fieldAccess The field access vertex.
	 */
	...
	/**
	 * Checks if a field access vertex was already processed by the global resolver.
	 * @param fieldAccess The field access vertex.
	 * @return boolean wrapper class with true if processed, false if not. null if the field access is not within the symbol table.
	 */
	public Boolean isProcessedFieldAccess( FieldAccess fieldAccess ){
		if( ( fieldAccess != null ) && ( fieldAccessMap.containsKey( fieldAccess ) ) ) return fieldAccessMap.get( fieldAccess );
		return null;
	}

	/**
	 * A structure holding references to the scopes of field access vertices.
	 * The data (Vertex) for every key (FieldAccess) is the scope of the field access.
	 */
	 ...
	 * @param fieldAccessVertex The field access vertex.
	 * @param scopeOfFieldAccess The scope of the field access.
	 */
	...
	public Vertex getScopeOfFieldAccess( FieldAccess fieldAccessVertex ){
		if ( ( fieldAccessVertex != null ) && ( scopesOfFieldAccessVertices.containsKey( fieldAccessVertex ) ) )
			return scopesOfFieldAccessVertices.get( fieldAccessVertex );
		return null;
	}

	/**
	 * Variable required for counting the amount of field access vertices that could not be resolved.
	 */
	...
	/**
	 * Variable required for counting the amount of field access vertices treated by the resolver.
	 */
...
//--- methods ---------------------------------------------------------------------------------------
	...
	/**
	 * A structure holding references to all found method declarations / definitions
	 * Key string in the outer hashmap is the fully qualified name of the class. Data for every key is a hashmap,
	 * where the key string is the name of the method. Data for every key in the inner hasmap is an arraylist of
	 * method declarations / definitions, as there may be overloaded methods in every class.
	 */
	 ...
	 * @param fullyQualifiedNameOfContainingType The fully qualified Name of the class which contains the method.
	 * @param constructorDefinition The vertex of the method declaration (or definition).
	 */
	public void addMethodDeclaration( String fullyQualifiedNameOfContainingType, MethodDeclaration methodDeclaration ){
		if ( ( fullyQualifiedNameOfContainingType != null ) && ( methodDeclaration != null ) ){
			String methodName = ( ( Identifier ) methodDeclaration.getFirstIsNameOfMethod( EdgeDirection.IN ).getAlpha() ).getName();
			if ( declaredMethodsInClasses.containsKey( fullyQualifiedNameOfContainingType ) ){
				if ( declaredMethodsInClasses.get( fullyQualifiedNameOfContainingType ).containsKey( methodName ) )
					declaredMethodsInClasses.get( fullyQualifiedNameOfContainingType ).get( methodName ).add( methodDeclaration );
				else{
					ArrayList< MethodDeclaration > methodsList = new ArrayList< MethodDeclaration >();
					methodsList.add( methodDeclaration );
					declaredMethodsInClasses.get( fullyQualifiedNameOfContainingType ).put( methodName, methodsList );
				}
			}
			else{
				ArrayList< MethodDeclaration > methodsList = new ArrayList< MethodDeclaration >();
				...
				declaredMethodsInClasses.put( fullyQualifiedNameOfContainingType, methodsHashMap );
			}
		}
	}

	/**
	 * Gets the methods (declarations as well as definitions) with a certain name defined in a class.
	 * @param fullyQualifiedNameOfContainingType The fully qualified Name of the class which contains the method.
	 * @param methodName The name of the method in the class.
	 * @return An arraylist of method declarations; null if there are none or the class cannot be found.
	 */
	public ArrayList< MethodDeclaration > getMethodDeclarations( String fullyQualifiedNameOfContainingType, String methodName ){
		if ( ( fullyQualifiedNameOfContainingType != null ) && ( methodName != null ) && declaredMethodsInClasses.containsKey( fullyQualifiedNameOfContainingType ) && declaredMethodsInClasses.get( fullyQualifiedNameOfContainingType ).containsKey( methodName ) )
			return declaredMethodsInClasses.get( fullyQualifiedNameOfContainingType ).get( methodName );
		return null;
	}

	/**
	 * A structure holding references to all found constructor definitions
	 * Key string is the fully qualified name of the class. Data for every key is an arraylist of constructor definitions,
	 * as there may be overloaded constructors in every class.
	 */
	 ...
	 * @param fullyQualifiedNameOfContainingType The fully qualified Name of the class which contains the constructor.
	 * @param constructorDefinition The vertex of the constructor definition.
	 */
	public void addConstructorDefinition( String fullyQualifiedNameOfContainingType, ConstructorDefinition constructorDefinition ){
		if ( ( fullyQualifiedNameOfContainingType != null ) && ( constructorDefinition != null ) ){
			if ( definedConstructorsInClasses.containsKey( fullyQualifiedNameOfContainingType ) )
				definedConstructorsInClasses.get( fullyQualifiedNameOfContainingType ).add( constructorDefinition );
			else{
				...
				definedConstructorsInClasses.put( fullyQualifiedNameOfContainingType, constructorsInCurrentClass );
			}
		}
	}
	 ...
	 * @param fullyQualifiedNameOfContainingType The fully qualified Name of the class which contains the constructor.
	 * @return An arraylist of constructor definitions; null if there are none or the class cannot be found.
	 */
	public ArrayList< ConstructorDefinition > getConstructorDefinitions( String fullyQualifiedNameOfContainingType ){
		if ( ( fullyQualifiedNameOfContainingType != null ) && definedConstructorsInClasses.containsKey( fullyQualifiedNameOfContainingType ) )
			return definedConstructorsInClasses.get( fullyQualifiedNameOfContainingType );
		return null;
	}

	/**
	 * Holds references to all method invocation vertices. Data for every key denotes if the field has already been processed by the resolver.
	 */
	private HashMap< MethodInvocation, Boolean > methodInvocationMap = new HashMap< MethodInvocation, Boolean >();

	/**
	 * @return The total amount of method invocations stored in the symbol table.
	 */
	public int amountOfMethodInvocations(){
		if( methodInvocationMap == null ) return 0;
		return methodInvocationMap.size();
	}

	/**
	 * Adds a method invocation to the symbol table.
	 * @param methodInvocation The method invocation vertex.
	 * @param scopeOfMethodInvocation The scope of the method invocation.
	 */
	public void addMethodInvocation( MethodInvocation methodInvocation, Vertex scopeOfMethodInvocation ){
		if ( methodInvocation != null ){
			methodInvocationMap.put( methodInvocation, Boolean.valueOf( false ) );
			if ( scopeOfMethodInvocation != null )
				scopesOfMethodInvocations.put( methodInvocation, scopeOfMethodInvocation );
		}
	}

	/**
     * @return All method invocations which are not connected to the structure representing their declaration.
     */
	...
	public void setMethodInvocationProcessed( MethodInvocation methodInvocation ){
		if( ( methodInvocation != null ) && ( methodInvocationMap.containsKey( methodInvocation ) ) )
			methodInvocationMap.put( methodInvocation, Boolean.valueOf( true ) );
	}

	/**
	 * Checks if a method invocation vertex was already processed by the global resolver.
	 * @param methodInvocation The method invocation vertex.
	 * @return boolean wrapper class with true if processed, false if not. null if the method invocation is not within the symbol table.
	 */
	...
	/**
	 * A structure holding references to the scopes of method invocations.
	 * The data (Vertex) for every key (MethodInvocation) is the scope of the method invocation.
	 */
	...
	public Vertex getScopeOfMethodInvocation( MethodInvocation methodInvocation ){
		if ( ( methodInvocation != null ) && ( scopesOfMethodInvocations.containsKey( methodInvocation ) ) )
			return scopesOfMethodInvocations.get( methodInvocation );
		return null;
	}

	/**
	 * Variable required for counting the amount of method invocation vertices that could not be resolved.
	 */
	...
	/**
	 * Variable required for counting the amount of method invocation vertices treated by the resolver.
	 */
	private int amountOfMethodInvocationsTreatedByResolver = 0;

	/**
	 * Increases the amount of method invocation vertices treated by the resolver.
	 */
...
//--- first type definition in currently parsed file ------------------------------------------------
	...
	/**
	 * Reference to vertex representing first type definition of currently parsed file.
	 */
	...
	/**
	 * @return Vertex representing first type definition of currently parsed file.
	 */
...
//----- type parameters in currently parsed file  -------------------------------------------------------
	...
	public void addTypeParameter( Vertex scope, String name, TypeParameterDeclaration typeParameterDeclarationVertex ){
		if( typeParameters == null ) typeParameters = new HashMap< Vertex, HashMap< String, TypeParameterDeclaration > >();
		if( typeParameters.containsKey( scope ) ){
			HashMap< String, TypeParameterDeclaration > temp = typeParameters.get( scope );
	...
	public boolean hasTypeParameter( Vertex scope, String name ){
		if( typeParameters == null ) return false;
		if( typeParameters.containsKey( scope ) ){
			HashMap< String, TypeParameterDeclaration > typeParametersInScope = typeParameters.get( scope );
			if( typeParametersInScope != null && typeParametersInScope.containsKey( name ) ) return true;
		}
		return false;
	}
...
//----- nested types in currently parsed file ---------------------------------------------------------------
    ...
    /**
     * Holds references to nested types in a file, collected by scope and fully qualified name.
     */
    private HashMap< String, Type > nestedTypes;

	public void addNestedClassDefinition( String fullyQualifiedName, ClassDefinition classDefinitionVertex ){
		addNestedType( fullyQualifiedName, classDefinitionVertex );
		// Check if it is protected, so it is accessible in same package.
		if( searchProtected( classDefinitionVertex.getIsModifierOfClassIncidences( EdgeDirection.IN ) ) ) addTypeDefinition( fullyQualifiedName, classDefinitionVertex );
//
//		Iterable< EdgeVertexPair< ? extends IsModifierOfClass, ? extends Vertex > > isModifierOfClassIncidences = classDefinitionVertex.getIsModifierOfClassIncidences( EdgeDirection.IN );
//		ArrayList< IsModifierOf> isModifierOfEdges = new ArrayList< IsModifierOf >();
//		for( EdgeVertexPair< ? extends IsModifierOfClass, ? extends Vertex > pair : isModifierOfClassIncidences )
//			isModifierOfEdges.add( ( IsModifierOf )pair.getEdge() );
//		if( searchProtected( isModifierOfEdges ) ) addTypeDefinition( fullyQualifiedName, classDefinitionVertex );
	}

	public void addNestedInterfaceDefinition( String fullyQualifiedName, InterfaceDefinition interfaceDefinitionVertex ){
		addNestedType( fullyQualifiedName, interfaceDefinitionVertex );
		// Check if it is protected, so it is accessible in same package.
		if( searchProtected( interfaceDefinitionVertex.getIsModifierOfInterfaceIncidences( EdgeDirection.IN )) ) addTypeDefinition( fullyQualifiedName, interfaceDefinitionVertex );

//		Iterable< EdgeVertexPair< ? extends IsModifierOfInterface, ? extends Vertex > > isModifierOfInterfaceIncidences = interfaceDefinitionVertex.getIsModifierOfInterfaceIncidences( EdgeDirection.IN );
//		ArrayList< IsModifierOf> isModifierOfEdges = new ArrayList< IsModifierOf >();
//		for( EdgeVertexPair< ? extends IsModifierOfInterface, ? extends Vertex > pair : isModifierOfInterfaceIncidences )
//			isModifierOfEdges.add( ( IsModifierOf )pair.getEdge() );
//		if( searchProtected( isModifierOfEdges ) ) addTypeDefinition( fullyQualifiedName, interfaceDefinitionVertex );
	}

	public void addNestedAnnotationDefinition( String fullyQualifiedName, AnnotationDefinition annotationDefinitionVertex ){
		addNestedType( fullyQualifiedName, annotationDefinitionVertex );
		// Check if it is protected, so it is accessible in same package.
		if( searchProtected( annotationDefinitionVertex.getIsModifierOfAnnotationIncidences( EdgeDirection.IN ) ) ) addTypeDefinition( fullyQualifiedName, annotationDefinitionVertex );

//		Iterable< EdgeVertexPair< ? extends IsModifierOfAnnotation, ? extends Vertex > > isModifierOfAnnotationIncidences = annotationDefinitionVertex.getIsModifierOfAnnotationIncidences( EdgeDirection.IN );
//		ArrayList< IsModifierOf> isModifierOfEdges = new ArrayList< IsModifierOf >();
//		for( EdgeVertexPair< ? extends IsModifierOfAnnotation, ? extends Vertex > pair : isModifierOfAnnotationIncidences )
//			isModifierOfEdges.add( ( IsModifierOf )pair.getEdge() );
//		if( searchProtected( isModifierOfEdges ) ) addTypeDefinition( fullyQualifiedName, annotationDefinitionVertex );
	}

	public void addNestedEnumDefinition( String fullyQualifiedName, EnumDefinition enumDefinitionVertex ){
		addNestedType( fullyQualifiedName, enumDefinitionVertex );
		// Check if it is protected, so it is accessible in same package.
		if( searchProtected( enumDefinitionVertex.getIsModifierOfEnumIncidences( EdgeDirection.IN ) ) ) addTypeDefinition( fullyQualifiedName, enumDefinitionVertex );

//		Iterable< EdgeVertexPair< ? extends IsModifierOfEnum, ? extends Vertex > > isModifierOfEnumIncidences = enumDefinitionVertex.getIsModifierOfEnumIncidences( EdgeDirection.IN );
//		ArrayList< IsModifierOf> isModifierOfEdges = new ArrayList< IsModifierOf >();
//		for( EdgeVertexPair< ? extends IsModifierOfEnum, ? extends Vertex > pair : isModifierOfEnumIncidences )
//			isModifierOfEdges.add( ( IsModifierOf )pair.getEdge() );
//		if( searchProtected( isModifierOfEdges ) ) addTypeDefinition( fullyQualifiedName, enumDefinitionVertex );
	}

	/**
	 * Searches for modifier protected in a given list.
	 * @param isModifierOfEdges List of edges attached to a vertex representing modifiers.
	 * @return true if no visibility modifier or at least modifier protected was found, otherwise false.
	 */
	private boolean searchProtected( Iterable<? extends IsModifierOf> isModifierOfEdges ){
		for( IsModifierOf isModifierOfEdge : isModifierOfEdges ){
			Modifier modifierVertex = ( Modifier )isModifierOfEdge.getAlpha();
			Modifiers modifierType = modifierVertex.getType();
			if( modifierType == Modifiers.PROTECTED ) return true;
			if ( modifierType == Modifiers.PRIVATE ) return false; // It cannot be public because it is nested.
		}
	 ...
	 * @param fullyQualifiedName Fully qualified name of type.
	 * @param typeVertex Vertex representing nested type definition.
	 */
    private void addNestedType( String fullyQualifiedName, Type typeVertex ){
        if( nestedTypes == null ) nestedTypes = new HashMap< String, Type >();
        nestedTypes.put( fullyQualifiedName, typeVertex );
    }

	/**
	 * @return true if symbol table knows at leastone nested type, false otherwise.
	 */
	 ...
	 * @return true if symbol table knows the nested type given by fully qualified name.
	 */
	...
	/**
	 * @param fullyQualifiedName Fuly qualified name of nested typ definition to get.
	 * @return Vertex representing definition of nested type, null if none exists for given name.
	 */
	public Type getNestedType( String fullyQualifiedName ){
		if( hasNestedType( fullyQualifiedName ) ) return nestedTypes.get( fullyQualifiedName );
		return null;
	}
...
//----- type specifications in currently parsed file ------------------------------------------------
	...
	/**
	 * Holds vertices respresenting type specifications of the currently parsed file.
	 * Before next file is parsed vertices have to be put to collection unresolvedTypeSpecifications.
	 */
	...
	/**
	 * Adds a vertex representing a type specification of curently parsed file to symbol table.
	...
	public void addUnresolvedTypeSpecification( Vertex scope, QualifiedType qualifiedTypeVertex ){
		if( !unresolvedTypeSpecificationsInCurrentlyParsedFile.containsKey( scope ) ){
			...
			unresolvedTypeSpecificationsInCurrentlyParsedFile.put( scope, qualifiedTypes );
		}
		else{
		    ArrayList< QualifiedType> qualifiedTypes = unresolvedTypeSpecificationsInCurrentlyParsedFile.get( scope );
		    qualifiedTypes.add( qualifiedTypeVertex );
		}
	...
	public ArrayList< QualifiedType > getUnresolvedTypeSpecificationsInCurrentlyParsedFile( Vertex scope ){
...
//----- type specifications which could not be resolved locally thus are saved for a global approach -----

	/**
	 * Collects vertices representing a type specification which could not be resolved locally.
	 */
	...
	public boolean hasUnresolvedTypeSpecifications(){
		...
		while( iterator.hasNext() ){
			Vertex scope = iterator.next();
			ArrayList< QualifiedType > qualifiedTypes = unresolvedTypeSpecifications.get( scope );
			if( !qualifiedTypes.isEmpty() ) return true;
		}
		return false;
	}
	...
	/**
	 * Decreases amount of unresolved type specifications in symbol table by given value.
	 * Only affects counter not size of collection in which vertices who represent a
	 * type specification are stored.
	 * @param amout Amount by which to decrease.
	 * @TODO relace by method which automatically decreases amount, when a type specification is resolved.
	 *       Hardly to encapsulate !!!
	 */
	public void decreaseAmountOfUnresolvedTypeSpecificationsBy( int amount ){
		amountOfUnresolvedTypeSpecifications -= amount;
	}

	private void putToUnresolvedTypeSpecifications( HashMap< Vertex, ArrayList< QualifiedType > > locallyUndefinedTypes ){
		//if( unresolvedTypeSpecifications == null ) unresolvedTypeSpecifications = new HashMap< Vertex, ArrayList< QualifiedType > >();
		Set< Vertex > keySet = locallyUndefinedTypes.keySet();
		Iterator< Vertex > iterator = keySet.iterator();
		while( iterator.hasNext() ){
			Vertex scope = iterator.next();
			ArrayList< QualifiedType > qualifiedTypes = locallyUndefinedTypes.get( scope );
			if( unresolvedTypeSpecifications.containsKey( scope ) ){
				//amountOfUnresolvedTypeSpecifications += qualifiedTypes.size();
	...
	public void addUnresolvedTypeSpecificationAfterParsing( Vertex scope, QualifiedType qualifiedTypeVertex ){
...
//--- type specifications which have been resolved --------------------------------
	...
	public void addResolvedTypeSpecification( QualifiedType qualifiedTypeVertex ){
		resolvedTypeSpecifications.put( qualifiedTypeVertex.getFullyQualifiedName(), qualifiedTypeVertex );
	}
...
//--- qualified names ---------------------------------------------------------------------------------------
    ...
    public void addQualifiedName( String fullyQualifiedName, QualifiedName qualifiedNameVertex ){
...
//--- primitive types -------------------------
    ...
    /**
     * Clears references to vertices which represent elements that have a local scope.
     * These are:
     * <ul>
    ...
    public void nextFile(){
		...
		if( hasUnresolvedTypeSpecificationsInCurrentlyParsedFile() ) putUnresolvedTypeSpecificationsInCurrentlyParsedFileToGlobalOnes(); // Save unresolved type specifications for later.
		...
		for( int counter = 0; counter < accessOfUndeclaredInternalVariables.size(); counter++ )
			fieldAccessMap.put( accessOfUndeclaredInternalVariables.get( counter ), Boolean.valueOf( false ) );
		accessOfUndeclaredInternalVariables.clear();
		if( nestedTypes != null ) nestedTypes.clear(); //@TODO clear maps in this hash map, too.
		if( typeParameters != null ) typeParameters.clear();
	}
}